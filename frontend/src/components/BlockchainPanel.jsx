...
function BlockchainPanel({ blocks }) {
  ...
  if (!blocks || blocks.length === 0) {
    return (
      <p className="rounded-lg border border-dashed border-white/25 bg-black/30 px-3 py-4 text-center text-base font-medium leading-relaxed text-zinc-200">
        ...
        return (
          ...
          >
            ...
            >
              ...
              <span className="text-base font-bold text-white">#{block.index}</span>

              {isGenesis && (
                <span className="ml-auto rounded-md bg-amber-500/25 px-2 py-0.5 text-sm font-bold text-amber-100">
                  Genesis
                </span>
              )}

              {!isGenesis && friendlyType && (
                <span className="ml-auto truncate text-sm font-semibold text-zinc-200" title={eventData.type}>
                  {friendlyType}
                </span>
              )}
            </button>
            ...
            {isExpanded && (
              <div className="space-y-2 border-t border-white/12 px-3 pb-3 pt-2 text-sm">
                <div className="break-all font-mono text-base leading-relaxed text-zinc-200">
                  <span className="font-sans text-sm font-bold uppercase tracking-wide text-zinc-400">Hash </span>
                  {block.hash}
                </div>
                <div className="break-all font-mono text-base leading-relaxed text-zinc-200">
                  <span className="font-sans text-sm font-bold uppercase tracking-wide text-zinc-400">Prev </span>
                  {block.previousHash}
                </div>

                {eventData && (
                  <div className="space-y-1.5 rounded-md border border-white/12 bg-black/40 p-3 text-base">
                ...
                {isGenesis && (
                  <div className="rounded-md border border-amber-400/30 bg-amber-950/60 px-3 py-2 text-base font-medium text-amber-100">
